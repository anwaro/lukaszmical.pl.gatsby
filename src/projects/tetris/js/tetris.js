...
function initCanvas() {
    ...
    isMobile && right.addEventListener('touchstart', function () {
        LR(true)
    });
    !isMobile && right.addEventListener('mousedown', function () {
        LR(true)
    });
    ...
    isMobile && left.addEventListener('touchstart', function () {
        LR(false)
    });
    !isMobile && left.addEventListener('mousedown', function () {
        LR(false)
    });
    ...
    isMobile && down.addEventListener('touchstart', function () {
        downPress = true;
        MoveCube()
    });
    !isMobile && down.addEventListener('mousedown', function () {
        downPress = true;
        MoveCube()
    });
...
function MoveCubeLR(where) {
    ...
    if (where && right === 0) for (let i2 = 0; i2 < 4; i2++) AllBlock[NumberCube].poss[i2][0]++;
    if (!where && left === 0) {
